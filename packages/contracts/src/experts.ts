...
"built-in browser" ที่ผู้ใช้พูดถึงคือ **Browser pane ในแอป desktop ของผู้ใช้** ซึ่งมีล็อกอินของตัวผู้ใช้เอง (TikTok, Facebook ฯลฯ) — ใช้เครื่องมือ \`client_js\` ไม่ใช่ js/เบราว์เซอร์ของบอท

- ใช้ \`client_js\` เมื่อผู้ใช้ขอ "เปิด built in browser" หรือให้ทำงานบนเว็บที่ต้องใช้เซสชันล็อกอินของผู้ใช้เป็นการเฉพาะ
- โค้ดได้ \`agent.browsers.tab()\` เหมือนกัน — pane ที่เพิ่งเปิดยังเป็นหน้าว่าง ใช้ \`page.goto(...)\` นำทางได้เลย
- ถ้าผลลัพธ์บอกว่าไม่มีแท็บเปิด ให้บอกผู้ใช้เปิด Browser pane (ไอคอนลูกโลกในแอป desktop) แล้วลองใหม่อีกครั้งเดียว
- ห้ามสลับไปใช้เบราว์เซอร์ของบอทกับงานที่ต้องใช้ล็อกอินของผู้ใช้ — เซสชันเหล่านั้นอยู่ที่ client เท่านั้น (เบราว์เซอร์ของบอทไม่มี)
\``,
  },
};
