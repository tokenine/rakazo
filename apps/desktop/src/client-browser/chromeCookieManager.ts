...
import type { ChromeBrowserDataImportError } from "./zcode-shims.js";
...
export interface BrowserDataLogger {
      ...
      try {
        ...
        const issue =
          error instanceof WindowsChromeAppBoundImportError
            ? error.code
            : (String(
                (error as { code?: string })?.code ?? "chrome_cookie_app_bound_decryption_failed",
              ) as ChromeBrowserDataImportError);
